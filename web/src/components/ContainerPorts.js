...
export default function ContainerPorts({ ports, hideOnEmptyPorts }) {
        const renderNoContainerPorts = () => {
            return (<NoPortsFoundMessage>No container ports has been exposed/mapped.</NoPortsFoundMessage>);
        }
            ...
            <div>
                {allPublishedPorts.map(port => (
                    <div key={port}>
                        ...
                        {ports.published[port] && ports.published[port].map((portItem, idx) => (
                            <div key={idx}>
                                    <div key={idx}>
                                    <PortInfo>
                                    <PortHighlight>{port}</PortHighlight> (container) -> <PortHighlight>{portItem.HostPort}</PortHighlight> ({portItem.HostIp === "127.0.0.1" || portItem.HostIp === "0.0.0.0" ? `localhost` : portItem.HostIp})
                                    </PortInfo> 
                                    </div>
                            </div>
                        ))}
                        {(!ports.published[port] && port) && (
                            <PortInfo>
                                <PortHighlight>{port}</PortHighlight> (container) is exposed, but is not mapped to any port on the host.
                            </PortInfo>
                        )}
                    </div>
                ))}
